...
(function() {
	...
	function createTransInfoWnd(contents) {
		...
		iw.addEventListener("open", function() {
			if(baidu.G("trans_prev")) {
				baidu.on(baidu.G("trans_prev"), "click", function() {
			...
			if(baidu.G("trans_next")) {
				baidu.on(baidu.G("trans_next"), "click", function() {
					if(nextTransCbk && typeof nextTransCbk == "function") {
						nextTransCbk(curIndex + 1, obj)
					}
				})
		...
		_renderList: function() {
			if(this._opts.renderOptions.panel && this._opts.renderOptions.panel.appendChild && this._results) {
				...
				if(status == BMAP_STATUS_SUCCESS) {
					for(var i = 0, l = this._results.getCurrentNumPois(); i < l; i++) {
						var item = this._createItem(i);
						resultsList.appendChild(item)
					}
				}
				if(status == BMAP_STATUS_CITY_LIST) {
					for(var i = 0, l = Math.min(6, this._results.getCityList().length); i < l; i++) {
	...
	baidu.extend(TransitRoute.prototype, {
		...
		_renderPlan: function(index) {
			...
			staMkr.addEventListener("click", function() {
				me._select(0)
			});
			...
			endMkr.addEventListener("click", function() {
				me._selectLast()
			});
			...
			for(var i = 0, l = plan.getNumLines(); i < l; i++) {
				...
				(function() {
					var curIndex = i;
					onMkr.addEventListener("click", function() {
						me._select((curIndex + 1) * 2 - 1)
					});
					offMkr.addEventListener("click", function() {
		...
		_renderList: function() {
			if(this._opts.renderOptions.panel && this._opts.renderOptions.panel.appendChild && this._results && this._results.getNumPlans() > 0) {
				...
				for(var i = 0, l = this._results.getNumPlans(); i < l; i++) {
	...
	baidu.extend(DWRoute.prototype, {
		...
		_processRaw: function() {
			...
			var polylinePoints = [];
			this._boundsPoints = [];
			for(var i = 0, l = kps.length - 1; i < l; i++) {
				var curKps = kps[i];
				var curRss = rss[i];
				var preRss = rss[i - 1] || {};
				var nxtRss = rss[i + 1];
				...
				if(cur) {
					for(var j = 0, ll = cur.points.length; j < ll; j++) {
		...
		_renderList: function() {
			if(this._opts.renderOptions.panel && this._opts.renderOptions.panel.appendChild && this._results && this._results.getNumPlans() > 0) {
				...
				var divSta = BaseRoute._genTitle(this._results.getStart().title, 'Instance("' + this.guid + '")._select(0)', 0);
				var tables = ["<table style='font:12px arial,simsun,sans-serif;border-collapse:collapse' cellpadding='0' cellspacing='0' border='0'>"];
				for(var i = 0, l = route.getNumSteps(); i < l; i++) {
					var bg = "";
	...
	baidu.extend(Popup.prototype, {
		...
		initialize: function() {
			...
			baidu.on(this.button, "click", function(e) {
	...
	baidu.extend(TrafficControl.prototype, {
		initialize: function(map) {
			...
			map.addEventListener("mapcontainerresize", function(e) {
				showMarker(e)
			});
		...
		_getTime: function() {
			...
			this._getCityCode(function(code) {
				for(var i = 0; i < aid.length; i++) {
					if(code == aid[i].code) {
						scriptRequest(mType.tileUrls[0] + aid[i].name + ".js?newmap=1", function() {
							me.updateTime(aid[i].name)
						}, "traffic_js");
						return
					}
				}
				scriptRequest(mType.tileUrls[0] + "beijing.js?newmap=1", function() {
		...
		addMarker: function() {
			...
			for(var k in mks) {
				if(mks[k].domElement == null) {
					mks[k].addEventListener("click", function() {
				...
				if(json && json.result && json.result.type == RETURN_TYPE_REVERSE_GEOCODER) {
					...
					if(result.error == 0) {
						...
						if(surrounds && surrounds.length) {
							for(var i = 0, l = surrounds.length; i < l; i++) {
								var localPoi = {},
		...
		if(usePoint) {
			switch(ret.type) {
				...
				case GEO_TYPE_LINE:
					ret.points = [];
					var curGeo = r.geo[0];
					for(var i = 0, l = curGeo.length - 1; i < l; i += 2) {
